...
@Component({
  selector: 'app-clearing-unit',
  templateUrl: './clearing-unit.component.html',
  styleUrls: ['./clearing-unit.component.css']
})
export class ClearingUnitComponent implements OnInit {
  ...
  submit(formValues) {
    this.setupService.addLogisticsUnits(formValues).subscribe(
      res => {
        this.successMsg = "Clearing Unit Added";
        this.showSuccess();
        setTimeout(function () {
          this.isShow = false;
          this.route.navigate(['/app/setup/logistic-unit/ViewClearing'])
        }.bind(this), 2000);
      }, err => {
