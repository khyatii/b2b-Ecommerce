...
@Component({
  selector: 'app-warehouse-unit',
  templateUrl: './warehouse-unit.component.html',
  styleUrls: ['./warehouse-unit.component.css']
})
export class WarehouseUnitComponent implements OnInit {
  ...
  submit(formValues) {
    this.setupService.addLogisticsUnits(formValues).subscribe(
      res => {
        this.successMsg = "Warehouse Unit Added";
        this.showSuccess();
        setTimeout(function () {
          this.isShow = false;
          this.route.navigate(['/app/setup/logistic-unit/ViewWarehouse'])
        }.bind(this), 2000);
      }, err => {
