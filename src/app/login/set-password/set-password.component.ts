...
import { PasswordService } from './set-password.service';
...
export class SetPasswordComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.passwordService.checkTokenExpire(this.value).subscribe(res=>{
      if(res){
      }
    },
    (err)=>{
  ...
  get txtPassword()  {
   ...
   submit(formValue){
     ...
     this.passwordService.postPassword(formValue).subscribe(res=>{
